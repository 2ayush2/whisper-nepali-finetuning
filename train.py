...
class WhisperFinetuner:
    ...
    def load_model(self):
        ...
        # Model & SpecAugment Parameters
        model_config.suppress_tokens = []
        model_config.use_cache = False
        model_config.apply_spec_augment = False
    ...
    def prepare_data(self):
        ...
        self.train_dataset = split_data["train"].map(format_fn,remove_columns=split_data["train"].column_names
        ).filter(remove_bad_examples)
        self.test_dataset = split_data["test"].map(format_fn, remove_columns=split_data["test"].column_names
        ).filter(remove_bad_examples)
